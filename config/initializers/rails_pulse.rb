RailsPulse.configure do |config|
  ...
  # Enable or disable Rails Pulse. Instrumentation stays off in the test suite so
  # tests don't require the rails_pulse_* tables and aren't measured.
  ...
  # Enable or disable background job tracking. drift runs feed-refresh and cleanup
  # jobs on Solid Queue, so track them.
  ...
  # In production, Rails Pulse data lives in its own `rails_pulse` database
  # (see config/database.yml). In development/test it shares the single primary
  # database, so connects_to is left unset there — mirrors how
  # config.solid_queue.connects_to is only set in config/environments/production.rb.
  ...
  # Enable/disable authentication. Always on — the dashboard exposes app internals.
  config.authentication_enabled = true

  # Gate the dashboard on a drift admin session (User#admin?). The proc runs in the
  # engine's controller context at request time, so cookies / redirect_to / main_app
  # are available, and the top-level Session model resolves the signed session cookie
  # exactly as app/controllers/concerns/authentication.rb does.
