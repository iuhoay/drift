...
Entries whose feed truncates or omits the body get a reader-triggered "load full text" action on the entry **show** page (`Entry::Scraper`, synchronous). The fetched copy lives in `entries.full_content` — never in `entries.content`, which is feed-owned and rewritten by `Feed::Refresher` on **every** poll, so anything scraped into it would be clobbered within 10 minutes. `full_content` is shared across subscribers (one reader's fetch fills the entry for everyone) and wins over `content` in `Entry#body`.
