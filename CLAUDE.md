...
- Recurring: `RefreshDueFeedsJob` every 10 min via `config/recurring.yml`
...
Vanilla Rails philosophy — rich models, thin controllers, no service-object layer. The exceptions are `Feed::Refresher` (HTTP fetch + parse), `Subscription::Subscribing` (subscribe flow), and `SavedItem::Fetcher` (full-article fetch).
