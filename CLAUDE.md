...
RSS reader: `Feed` → `Subscription` (user↔feed, optional `custom_title` + `category` + `watched`) → `Entry` → `UserEntry` (per-user `read_at`/`starred_at`, created lazily). `category` is a user-typed label on the join (not on the shared Feed); inbox/API filter it case-insensitively via `?category=`, a write that only differs in casing adopts the existing label, and `PATCH /api/subscriptions/:id` lets the CLI recategorize. `watched` is a per-subscription opt-in: after a feed's first successful fetch, newly created entries email the watcher (titles + links, not an unread digest, and never marked read). Toggle it via `POST`/`DELETE /subscriptions/:id/watch` (and the same under `/api`). Auth is the Rails 8 generator (DB-backed `Session`) plus a hand-written `RegistrationsController`, OAuth via OmniAuth (`Identity` — GitHub/Google), and long-lived `ApiToken`s for the browser extension's JSON API. Full-text search lives in the `Searchable` concern (shared by `Entry` and `SavedItem`), not on `Entry` directly — weighted `tsvector` (title A > summary B > content/full_content C > author D) rebuilt before_save. CJK indexing stores both unigrams and overlapping 2-grams, while queries use 2-grams except for a lone CJK character; this preserves precise sub-word matching and lets single-character searches match without a server-side parser.
...
`SavedItem` is a separate read-later library (browser-extension / pasted URLs) that fetches the **full** article via `SavedItem::Fetcher` and carries its own `read_at`/`starred_at` through `Readable`. `ArticleExtractor` holds the shared Readability-extract/sanitize/absolutize step used by both `SavedItem::Fetcher` and `Entry::Scraper`.

Entries whose feed truncates or omits the body get a reader-triggered "load full text" action on the entry **show** page (`Entry::Scraper`, run in the background by `EntryScrapeJob`, which broadcasts the result back to the page over the entry's Turbo Stream). The fetched copy lives in `entries.full_content` — never in `entries.content`, which is feed-owned and rewritten by `Feed::Refresher` on **every** poll, so anything scraped into it would be clobbered within 10 minutes. `full_content` is shared across subscribers (one reader's fetch fills the entry for everyone) and wins over `content` in `Entry#body`.

**Star vs. save — intentionally NOT bridged.** Do not add a "save feed entry to read later" action. It overlaps with entry **star** (`UserEntry.starred_at`), which is the single "come back to this feed entry" affordance. Star is a lightweight flag on a feed-owned `Entry`; `SavedItem` is a personal, durable, full-text copy. They were deliberately kept separate (decision 2026-06-20). The "feed truncates content" case that once justified revisiting is now served by the inline full-text fetch above (decision 2026-07-14), so there is no remaining reason to bridge them.

Vanilla Rails philosophy — rich models, thin controllers, no service-object layer. The deliberate exceptions are the POROs under `app/models/**` that do HTTP fetch / parse / orchestration — `Feed::Refresher`, `Feed::Discovery`, `Feed::Bilibili`, `Feed::PublicAddressGuard` (a Faraday middleware), `Subscription::Subscribing`, `SavedItem::Fetcher`, `Entry::Scraper`, and `ArticleExtractor` — each typically paired with an Active Job in `app/jobs`.
